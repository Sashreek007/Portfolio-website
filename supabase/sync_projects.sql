...
INSERT INTO projects (name, description, sort_order)
SELECT 'Durable Execution Engine (Latch)', 'A durable execution engine for AI agent workflows. A worker claims one step at a time under a lease, a model decides what happens next, and every decision is recorded as a durable fact the moment it is made — so a machine that dies mid-step recovers by reading facts forward instead of re-running work. Tool calls that spend money or can''t be undone are gated behind a human approval. Currently in development.', 2
WHERE NOT EXISTS (SELECT 1 FROM projects WHERE name = 'Durable Execution Engine (Latch)');

UPDATE projects SET
  description = 'A durable execution engine for AI agent workflows. A worker claims one step at a time under a lease, a model decides what happens next, and every decision is recorded as a durable fact the moment it is made — so a machine that dies mid-step recovers by reading facts forward instead of re-running work. Tool calls that spend money or can''t be undone are gated behind a human approval. Currently in development.',
  github_url  = NULL,
  demo_url    = NULL,
  image_url   = '/projects/latch/workflow-trace.webp',
  video_url   = NULL,
  gallery     = '[{"url":"/projects/latch/workflows.webp","caption":"UI prototype — workflow operations","alt":"Dark dashboard prototype titled ''Workflow operations''. Four counters across the top track active, ready, needs-review and recovered workflows. A live workflow table lists each run with its status, phase, step, spend and age, and a right-hand rail shows the selected workflow''s definition, worker, lease token and event count, its latest checkpoint, and a panel showing the four invariants as verified. All values are sample data."},{"url":"/projects/latch/workflow-trace.webp","caption":"UI prototype — workflow trace","alt":"Dark dashboard prototype showing the trace of a single workflow as a chronological event timeline: a lease being claimed, a model decision recorded with its provider, token count and cost, an intent recorded with an idempotency key, a worker handover to a second worker, and a deduplicated outcome noting no second provider call. A right-hand rail breaks out the budget envelope, the lease with its heartbeat and fencing token, the model decision, and the effect guarantee. All values are sample data."},{"url":"/projects/latch/approvals.webp","caption":"UI prototype — approval queue","alt":"Dark dashboard prototype titled ''Approval queue'', listing risk-gated actions awaiting a decision with their due times, customer and risk level. The detail pane shows ticket facts, a narrow audit timeline, and side-by-side proposer and critic panels in which one model recommends approving a refund and a second escalates it for manual review, each with its rationale and evidence. A footer notes the workflow is suspended holding rows only. All values are sample data."},{"url":"/projects/latch/invariants.webp","caption":"UI prototype — invariant sweep","alt":"Dark dashboard prototype titled ''Correctness verification''. Four cards show the invariants — no duplicate effects, no lost workflows, single owner, budget ceiling — each marked verified. Panels below report a seeded invariant sweep, the mechanism backing each invariant, recently failing fixtures that were fixed, and a crash-coverage list marking which points in the step loop have been exercised. All values are sample data."},{"url":"/projects/latch/cost-reliability.webp","caption":"UI prototype — cost and reliability","alt":"Dark dashboard prototype titled ''Cost & reliability''. Top cards track model spend, average spend per workflow, routing savings and budget breaches, with sparklines. Panels below break spend down by workflow phase and provider, compare routing strategies against their estimated spend, list recent recovery evidence, and show a budget envelope in which reserved plus spent stays within the ceiling. All values are sample data."}]'::jsonb,
  highlights  = ARRAY['Queue, events and state commit in one Postgres transaction, so there is no dual-write window where work is queued but the state never lands.','Workers hold a lease with a fencing token, so a zombie that wakes up after its lease expired cannot write over the worker that replaced it.','Idempotency keys name the logical effect rather than the attempt, so a retry after a crash settles the same effect instead of firing it a second time.','Correctness is meant to be proven rather than asserted: the four invariants are pure functions run over the whole event log across seeded crash schedules.','Cloud-native from the first week — ECS Fargate, RDS Postgres, SQS and Bedrock, defined in CDK and deployed by GitHub Actions over OIDC.']::text[],
  stack       = ARRAY['TypeScript','Node.js','PostgreSQL 17','AWS ECS Fargate','Amazon SQS','Amazon Bedrock','OpenAI','AWS CDK','Docker','GitHub Actions']::text[],
  status      = 'building',
  year        = 2026,
  is_best     = true,
  is_current  = true,
...
UPDATE projects SET
  description = 'AI-assisted job workflow platform that matches roles to your profile, generates tailored resumes, and supports browser-assisted applications while keeping the user in control.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['React','TypeScript','FastAPI','Python','Playwright','SQLite','Gemini API']::text[],
  status      = 'shipped',
  year        = 2025,
  is_best     = true,
  is_current  = false,
...
UPDATE projects SET
  description = 'Chrome extension that converts doomscrolling into measurable distance, coins, and multiplayer battles with local-first tracking, Supabase sync, and personalized AI feedback.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['TypeScript','React','TailwindCSS','Supabase','PostgreSQL','Chrome Extension']::text[],
  status      = 'shipped',
  year        = 2025,
  is_best     = true,
  is_current  = false,
...
UPDATE projects SET
  description = 'Full-stack auction simulation platform modeling international resource trading with Vickrey auction mechanisms across 50+ simulated countries.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['FastAPI','React','PostgreSQL','Python','TypeScript']::text[],
  status      = 'shipped',
  year        = 2025,
  is_best     = true,
  is_current  = false,
...
UPDATE projects SET
  description = 'Deployed scam detection bot that identifies and removes malicious messages in real time with low-latency inference.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['Python','Discord.py','Hugging Face']::text[],
  status      = 'active',
  year        = 2025,
  is_best     = true,
  is_current  = false,
...
UPDATE projects SET
  description = 'Gesture-controlled arcade game with webcam-based hand tracking at 60 FPS and dynamic difficulty scaling.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['Python','MediaPipe','Pygame','OpenCV']::text[],
  status      = 'shipped',
  year        = 2024,
  is_best     = true,
  is_current  = false,
...
UPDATE projects SET
  description = 'Automated profile analysis pipeline combining web scraping and LLM processing to generate structured summaries.',
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['Python','LangChain','OpenAI API']::text[],
  status      = 'shipped',
  year        = 2024,
  is_best     = false,
  is_current  = false,
...
UPDATE projects SET
  ...
  gallery     = '[]'::jsonb,
  highlights  = '{}',
  stack       = ARRAY['Python','LangChain','LangGraph','MCP']::text[],
  status      = 'building',
  year        = 2025,
  is_best     = false,
  is_current  = true,
